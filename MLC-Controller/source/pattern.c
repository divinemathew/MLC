	...
	while (1) {
		if (xQueueReceive(pattern_control_queue, &received_config, 0)) {
			if (received_config.control_mode == NOP) {
				...
				set_pwm_frequency(config.refresh_rate);
				xTimerChangePeriod(color_change_timer,config.color_change_rate, 0);
...
{QueueHandle_t device_status_queue = get_queue_handle(DEVICE_STATUS_QUEUE);
	...
	xQueueSend(device_status_queue,&going_up,0);
}
...
void set_pwm_frequency(uint16_t frequency)
{
    ...
    ftm_chnl_pwm_signal_param_t pwm_config[3];
